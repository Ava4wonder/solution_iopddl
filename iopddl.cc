...
absl::StatusOr<TotalCost> Evaluate(const Problem& problem,
  ...
  if (problem.usage_limit) {
    std::cout << "Problem usage limit: " << *problem.usage_limit << "\n";
    // Compute the maximum total usage over all time indices
    TotalUsage max_total_usage = *std::max_element(total_usages.begin(), total_usages.end());
    std::cout << "Max total usage of solution: " << max_total_usage << "\n";

...
// 合并有共同节点的cluster
std::vector<std::set<int>> mergeClusters(const std::vector<std::set<int>>& clusters) {
    UnionFind uf;

    std::cout << "Enter merge function" << std::endl;
    // 遍历所有的cluster，合并有共同节点的cluster
    for (size_t i = 0; i < clusters.size(); ++i) {
        // std::cout << "Nodes in cluster " << i << ": ";
        //             for (int clusterNode : clusters[i]) {
        //                 std::cout << clusterNode << " ";
        //             }
        //             std::cout << std::endl;
        for (size_t j = i + 1; j < clusters.size(); ++j) {
            // std::cout << "Nodes in cluster " << j << ": ";
            //         for (int clusterNode : clusters[j]) {
            //             std::cout << clusterNode << " ";
            //         }
            //         std::cout << std::endl;
            for (int node : clusters[i]) {
                if (clusters[j].count(node)) {
                    uf.unite(i, j);
                    std::cout << "Merging cluster " << i << " and cluster " << j << " due to common node " << node << std::endl;
                    std::cout << "Nodes in cluster " << i << ": ";
                    for (int clusterNode : clusters[i]) {
                        std::cout << clusterNode << " ";
                    }
    ...
    // 将每个cluster合并到对应的根集合中
    std::unordered_map<int, std::set<int>> mergedClustersMap;
    for (size_t i = 0; i < clusters.size(); ++i) {
        int root = uf.find(i);
        for (int node : clusters[i]) {
            mergedClustersMap[root].insert(node);
        }
    }

    // 将合并后的集合转换为向量
    std::vector<std::set<int>> mergedClusters;
    for (const auto& pair : mergedClustersMap) {
        mergedClusters.push_back(pair.second);
    }
...
// Cluster create_clusters(const Problem &problem) {
    ...
    for (const auto &edge : problem.edges) {
        ...
        if (max_cost - min_cost >= 1e18) {
            ...
            // big_macro_edges.emplace_back(dst, src);  // Ensure bidirectionality
            if (src == 1250 or dst == 1250 or src == 31770 or dst == 31770){
    ...
    std::vector<std::set<int>> rawclusters;
    ...
    for (int node : big_macro_nodes) {
        if (!visited.count(node)) {
            ...
            rawclusters.push_back(std::move(cluster));
        }
    }

    auto clusters = mergeClusters(rawclusters);
    ...
    if (cluster_map.data.find(138) != cluster_map.data.end()) {
        std::cout << "Cluster ID: 138 contains the following src keys:\n";
        for (const auto& src_pair : cluster_map.data[138]) {
            std::cout << "  src: " << src_pair.first;
            
            if (src_pair.second.empty()) {
                std::cout << " (empty)";
            }

            std::cout << "\n";

            for (const auto& strategy_pair : src_pair.second) {
                std::cout << "    strategy_group: " << strategy_pair.first << "\n";
            }
        }
    }

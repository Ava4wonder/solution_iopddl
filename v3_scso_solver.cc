...
std::pair<std::vector<double>, std::unordered_map<int, int>> ComputeCriticality(
  ...
  return {crit, minCostOptIndex};  // Return both crit values and minCost option indices
}
...
Solution RunSCSO(const Problem &problem, absl::Time start_time, absl::Duration timeout) {
  ...
  auto [critScores, minCostIndices] = ComputeCriticality(problem, safestSolution, safestCost);
  ...
  std::vector<int> unchangedCriticalNodes = criticalNodes;
//   std::vector<int> criticalNodes{541, 694, 579, 590, 726, 488, 642, 649, 276, 106, 736, 196, 247, 388, 132, 555, 187, 495, 535, 389, 262, 413, 576, 45, 203, 738, 601, 572, 48, 409, 258, 469, 708, 149, 87, 424, 421, 621, 606, 676, 200, 12, 723, 719, 470, 184, 609, 624, 500, 473, 84, 523, 643, 499, 650, 14, 740, 153, 463, 663, 526, 510, 652, 509, 679, 502, 659, 653, 71, 74, 229, 146, 496, 144, 114, 164, 506, 477, 273, 483, 598, 569, 567, 452, 241, 11, 141, 446, 480, 545, 16, 686};
  ...
  for (int coarseiter = 0; coarseiter < MAX_COARSE_ITERATIONS; ++coarseiter) {
    ...
    // bool feasible = true;
    // for (const auto &[node_idx, prev_opt_idx] : modified_nodes) {
    //     if (!IsFeasibleUpdated(node_idx, prev_opt_idx, newSol, union_mem_dict, nodeToUnions, problem)) {
    //         feasible = false;
    //         break;
    //     }
    // }
    ...
    if (IsFeasibleUpdated(node_index, pre_option_idx, newSol, union_mem_dict, nodeToUnions, problem) && (bestCost - cost > bestCost * 0.001)) {
        // double cost = ComputeCost(newSol, problem);
        // if () {
        ...
        if (node_index == 14339) {
                std::cout << "//// 14339 ### " << IsFeasibleUpdated(node_index, pre_option_idx, newSol, union_mem_dict, nodeToUnions, problem) << "\n";
            }
        ...
        for (int i = 0; i < problem.nodes[node_index].strategies.size(); ++i){
            ...
            if (node_index == 14339 && i == 8) {
                ...
                std::cout << "@@ 14339 ### Feasible: " << (is_feasible ? "Yes" : "No")
                        << " | Best Cost: " << bestCost
...
//   int segment_end = unchangedCriticalNodes.empty() ? 0 
  ...
  for (int iter = 0; iter < MAX_ITERATIONS; ++iter) {
    ...
    //         if (!unchangedCriticalNodes.empty()) {
    //             candidateNodes.assign(unchangedCriticalNodes.begin() + segment_start, unchangedCriticalNodes.begin() + segment_end);
    //         }
    //     }
